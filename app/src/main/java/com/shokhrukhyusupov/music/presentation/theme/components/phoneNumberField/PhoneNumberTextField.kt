...
fun PhoneNumberTextField(
    ...
    val countries = remember {
        ...
        Locale.getISOCountries().mapNotNull { isoCode ->
            ...
            val dialCode = phoneUtil.getCountryCodeForRegion(
                isoCode
            )
            ...
            CountryInfo(
                ...
                name = countryLocale.getDisplayCountry(
                    Locale.getDefault()
                ),

                dialCode = "+$dialCode",

                flagEmoji = PhoneNumberUtils.isoToEmojiFlag(
                    isoCode
                )
            )
        }.sortedBy {
    ...
    val maxLength = remember(mask) {
        PhoneMasks.getMaxLengthFromMask(
            mask
        )
    }

    val placeholderColor = MaterialTheme.colorScheme.onSurfaceVariant.copy(alpha = 0.35f)
    ...
    BasicTextField(
        ...
        onValueChange = { newValue ->
            ...
            val digitsOnly = PhoneNumberUtils.digitsOnly(
                newValue
            )
